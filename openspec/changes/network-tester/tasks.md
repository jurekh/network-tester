...
- [ ] 5.1 Implement `mtu_validator.py`: select the deterministic source representative for the local rack (lexicographically lowest in-scope data `system_id`); skip with `skip_reason: not-rack-representative` on all other nodes; from each source representative, derive one representative in-scope data target per remote rack and run `ping -M do -s <size> -c 1 -W 2 <target>` at 1472 and 8972 byte ICMP payload sizes; record observed rack-pair path MTU as payload size + 28 without pass/fail comparison
...
- [ ] 6.1 Implement `bgp_inference.py`: select the deterministic source representative for the local rack (lexicographically lowest in-scope data `system_id`); skip with `skip_reason: not-rack-representative` on all other nodes; from each source representative, derive the remote rack set from in-scope machines (distinct `rack` values of machines with `role: data` where rack != local rack); probe the representative in-scope data node per remote rack (lexicographically lowest `system_id`) via ICMP; if it fails, probe one fallback in-scope data node in that rack (next lexicographic `system_id`) before flagging rack-pair unreachable
...
- [ ] 9.5 Separate BGP inference findings by confidence: `diagnosis_confidence: inferred` entries go into `inferred_failures` (never `definitive_failures`); `diagnosis_confidence: inconclusive` entries go into `inconclusive_checks`; reconcile the two directions of each rack-pair link before assigning final confidence: both directions fail -> one `inferred_failures` entry per rack-pair link with `observed_by` list and per-node traceroute data preserved; one direction fails with healthy reverse path -> node-scoped warning naming the source representative, no rack-pair failure; one direction missing (source rep in `missing_nodes` or timed out) -> `inconclusive_checks` entry noting the unavailable reverse direction
...
- [ ] 9.9 Implement `observations` list in the JSON report: include MTU probe results (source node, target node, `observed_path_mtu_bytes` or `null` with `status: "inconclusive"`) as informational entries with no pass/fail verdict; add a `warnings` entry when both directions of a rack-pair link report disagreeing MTU values; include symmetric-bond-swap findings here if MAC manifest was provided
- [ ] 9.10 Implement phase-1 health gating: when a source representative has definitive bond-validator or vlan-neighbor-validator failures, move its bgp-inference findings to `inconclusive_checks` with a reference to the node fault and annotate that rack's outbound MTU observations with the same reference
...
- [ ] 11.6 Unit test MTU validator: mock representative and non-representative local nodes plus ICMP probe responses at different payload sizes; assert non-representatives skip, representatives probe one target per remote rack, payload+28 MTU recording, binary search, and inconclusive handling (covered in 5.5)
- [ ] 11.7 Unit test BGP inference: mock representative and non-representative local nodes, representative/fallback target ICMP outcomes, and traceroute subprocess output for each hop classification; assert non-representatives skip and rack-pair scoped findings are emitted only by source-rack representatives (covered in 6.5)
...
- [ ] 11.9 Unit test report generator: provide synthetic probe outputs from 3 nodes; assert bidirectional edge confirmation, skip grouping from `in_scope: false` peers, and diff against expected topology; cover directional rack-pair reconciliation (both-directions-fail -> one inferred failure; one-direction-fails-with-healthy-reverse -> source-node warning, no rack-pair failure; one-direction-missing -> inconclusive) and phase-1 health gating of a source representative's BGP findings
...
- [ ] 13.14 Extend golden fixtures with directional reconciliation cases: both-directions-fail, one-direction-fails-with-healthy-reverse-path, one-direction-missing, a source representative with definitive phase-1 failures, and asymmetric directional MTU observations; assert the final report classification per case (inferred failure, source-node warning, inconclusive, MTU asymmetry warning)
