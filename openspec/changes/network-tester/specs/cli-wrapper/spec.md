...
After all units reach `active/idle` (or after `--wait-timeout` seconds, default 600), the CLI SHALL set the `probe-run-id` application config to the current timestamp string (`YYYYMMDD-HHMMSS`) via `juju config <app> probe-run-id=<value>`. Each invocation uses a unique value; this ensures re-runs are always distinguishable by charm units. The CLI wrapper owns the timeout decision; the charm does not implement a timer.
