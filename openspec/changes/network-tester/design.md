...
The timeout decision (how long to wait for units before proceeding with partial deployment) belongs in the CLI wrapper, not the charm. Juju hooks only fire on events; there is no reliable "sleep N seconds then fire" mechanism in charms. The CLI wrapper polls the Juju model for unit readiness and sets `probe-run-id: <YYYYMMDD-HHMMSS>` via `juju config` once all units reach `active/idle` or after `--wait-timeout` elapses. Every unit triggers its probe from its own `config-changed` hook (Juju delivers the config event to all units within seconds), which keeps the passive capture windows concurrent across units; the leader additionally writes the run-id to peer relation data as a catch-up trigger for units that missed the config event. Probing must not be driven solely by the leader's relation write: relation data commits only when the leader's hook exits, so relation-driven peers would start a full probe-duration after the leader and cross-unit passive observations (unexpected-l2-neighbor) would structurally miss the leader's traffic. Using a unique run-id string rather than a boolean ensures that re-runs (including `--reuse-model`) reliably trigger a new probe cycle without relying on units seeing a false->true transition.
...
- microOVN/OVN: rejected in favor of plain Open vSwitch (the `openvswitch-switch` deb, bare bridges, no OVN layer) for four reasons. (1) The overlay hides exactly what the tool probes: the validators check L2 details of physical access ports (per-port VLAN tags, LACP state toward host bonds, inter-rack link MTU), while OVN models logical switches over Geneve tunnels with no natural place for "wrong 802.1Q tag on an access port" or "switch-side bond stopped doing LACP". (2) Fault injection conflicts with OVN's control plane: the fault scripts retag OVS ports and alter OVS bonds directly via `ovs-vsctl`, but `ovn-controller` owns the integration bridge and reconciles its state from the southbound DB, so manual OVS-level changes get fought or sit outside what OVN manages; the needed faults are not expressible in the northbound model. (3) BGP and MTU testing need a real routed hop: bgp-inference and mtu-validator depend on FRR routers as traceroute-visible hops with eBGP sessions that can be shut down and an inter-rack link whose MTU can be lowered; OVN logical routers do not run BGP, and Geneve encapsulation changes effective MTU in ways that would contaminate MTU observations. (4) microOVN's value is a clustered OVN deployment across machines; this testbed is one disposable VM, so the clustering buys nothing and the overlay would have to be worked around
