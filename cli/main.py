...
from cli import maas_topology, representatives
...
def cmd_run(args):
    ...
    if args.reuse_model:
        print(
            "error: --reuse-model is not implemented yet (arrives with deployment support)",
            file=sys.stderr,
        )
        return 2
    ...
    if args.dry_run:
        return 0
    print("error: deployment is not implemented yet; use --dry-run", file=sys.stderr)
    return 2


def cmd_status(args):
    print(
        "error: status is not implemented yet (arrives with deployment support)", file=sys.stderr
    )
    return 2
